...
import { X, Calendar, Plus, Trash2, Check, ExternalLink, Type } from 'lucide-react';
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {/* TEXT SIZE — persisted to localStorage; scales document root font-size. */}
          <section className="mb-8">
            ...
            <p style={{
              ...
              For more comfortable reading on small screens.
            </p>
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: 'default', label: 'Default', sample: '0.95rem' },
                { id: 'large',   label: 'Large',   sample: '1.05rem' },
                { id: 'xlarge',  label: 'Extra',   sample: '1.2rem'  },
              ].map(opt => {
                const active = textScale === opt.id;
                return (
                  <button
                    key={opt.id}
                    onClick={() => onTextScaleChange && onTextScaleChange(opt.id)}
                    className="flex flex-col items-center justify-center gap-1 py-3 rounded transition-colors"
                    style={{
