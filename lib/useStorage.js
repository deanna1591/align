...
function groupByDate(rows) {
  const out = {};
  for (const r of rows) {
    const k = r.date || 'someday';
    if (!out[k]) out[k] = [];
    out[k].push(rowToTask(r));
  }
...
export function useStorage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'tasks', filter: `user_id=eq.${user.id}` },
        (payload) => {
          const t = rowToTask(payload.new); const date = payload.new.date || 'someday';
          setTasks(prev => {
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tasks', filter: `user_id=eq.${user.id}` },
        (payload) => {
          const t = rowToTask(payload.new); const newDate = payload.new.date || 'someday'; const oldDate = payload.old?.date || (payload.old?.date === null ? 'someday' : null);
          setTasks(prev => {
            const next = { ...prev };
  ...
  const addTask = useCallback(async (dKey, text) => {
    ...
    const id = newId();
    const newTask = { id, text, completed: false, started: false, notes: '', position, leftover: false };
    setTasks(prev => ({ ...prev, [dKey]: [...(prev[dKey] || []), newTask] }));
    const dbDate = dKey === 'someday' ? null : dKey;
    const { error: e } = await supabase.from('tasks').insert({ id, user_id: user.id, date: dbDate, text, position });
    if (e) {
  ...
  const moveTaskBetweenDays = useCallback(async (fromDate, toDate, taskId) => {
    ...
    setTasks(prev => {
      ...
      const next = { ...prev, [fromDate]: f, [toDate]: [...(prev[toDate] || []), { ...task, position: newPosition, leftover: false }] };
      if (!f.length) delete next[fromDate];
      return next;
    });
    const dbDate = toDate === 'someday' ? null : toDate;
    const { error: e } = await supabase.from('tasks').update({ date: dbDate, position: newPosition, leftover: false }).eq('id', taskId);
  ...
  // Move every UNCOMPLETED task dated before `toKey` onto `toKey`, flagged as
  // leftover, in a single state update + one batched DB write. Completed tasks
  // stay on their original day (history intact); 'someday' and future days are
  // left alone. Returns the ids that were rolled over.
  const rolloverIncomplete = useCallback(async (toKey) => {
    if (!user || !toKey || toKey === 'someday') return [];
    const ids = [];
    const moved = [];
    for (const k of Object.keys(tasks)) {
      if (k === 'someday' || k >= toKey) continue; // only real past dates
      for (const t of (tasks[k] || [])) {
        if (!t.completed) { ids.push(t.id); moved.push(t); }
      }
    }
    ...
    setTasks(prev => {
      const next = { ...prev };
      // strip incomplete tasks out of past days (keep completed for history)
      for (const k of Object.keys(next)) {
        if (k === 'someday' || k >= toKey) continue;
        const kept = (next[k] || []).filter(t => t.completed);
        if (kept.length) next[k] = kept; else delete next[k];
      }
      // append them to the target day, flagged leftover
      let base = (next[toKey] || []).length;
      const appended = moved.map((t, i) => ({ ...t, leftover: true, position: base + i }));
      next[toKey] = [...(next[toKey] || []), ...appended];
      return next;
    });
    const { error: e } = await supabase
      .from('tasks')
      .update({ date: toKey, leftover: true })
      .in('id', ids);
    if (e) { reportError(setError, 'Roll over tasks', e); setTasks(original); return []; }
    return ids;
  }, [user, tasks, supabase]);
